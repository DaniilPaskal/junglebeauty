...
const KittenGallery = () => {
  ...
  useEffect(() => {
    setFilters({
      colour: colours,
      father: parents
        .filter((cat) => cat.sex === "male")
        .map((cat) => {
          return cat.name;
        }),
      mother: parents
        .filter((cat) => cat.sex === "female")
        .map((cat) => {
          return cat.name;
        }),
      status: ["available"],
  ...
  const toggleAll = () => {
    if (
      ...
      filters["father"].length > 0 ||
      filters["mother"].length > 0 ||
  ...
  return (
    <>
      <FilterAccordion
        ...
        toggleAll={toggleAll}
        filters={filters}
        options={{ colours, statuses, sexes, parents }}
      />
      ...
      filters["mother"].length === 0 ? (
        <div className="filter-warning selection-warning">
          <h5>
            Please select at least one status, fur colour, gender, father, and
            mother to display kittens.
          </h5>
        </div>
      ) : (
      ...
      <div className="media-grid">
        {filterKittens(kittens, filters)
          .sort((a, b) => (a.date > b.date ? -1 : 1))
          .map((cat) => {
            return <CatCard key={cat.id} cat={cat} size="small" />;
          })}
      </div>
    </>
  );
