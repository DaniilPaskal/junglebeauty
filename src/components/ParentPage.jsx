...
const ParentPage = ({ sex }) => {
  ...
  if (cats) {
    cats = cats.parents.filter(
      (cat) => cat.sex === sex && cat.show !== "false"
    );
  } else {
    cats = [];
  }
  ...
  return (
    <main className="page-background paragraph-container">
      <h2 className="page-header">
        {sex === "male" ? "Kings" : "Queens"} of JungleBeauty, TICA and CCA
        registered!
      </h2>
      ...
      <Modal show={show} onHide={handleClose} size="lg">
        ...
        <Modal.Body>{cats && <ParentProfile cat={cat} />}</Modal.Body>
      </Modal>
    </main>
  );
};
